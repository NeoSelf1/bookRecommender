...
class EnhancedBookSearchManager {
    ...
    func getAdditionalBookFromGPT(for question: String, from previousResults: [RawBook]) async throws -> RawBook {
        ...
        while retryCount < maxRetries {
            do {
                ...
                retryCount += 1
                print("Retry attempt in getAdditionalBookFromGPT \(retryCount): \(error.localizedDescription)")
                continue
            }
        }
    ...
    private func getBookRecommendation(question:String,ownedBooks: [String]) async throws -> (recommendationFromOwned:[String],recommendationFromUnowned:[RawBook]) {
        ...
        while retryCount < maxRetries {
            do {
                guard
                    let system = loadEnv()?["PROMPT"],
                    let openAIApiKey = loadEnv()?["OPENAI_API_KEY"],
                    let url = URL(string: "https://api.openai.com/v1/chat/completions") else {
                    print("GPT Error in getBookRecommendation")
                    throw GPTError.noKeys
                }
                ...
                guard let jsonString = response.choices.first?.message.content,
                    ...
                    print("getBookRecommendation gpt error: \(response)")
                    throw GPTError.invalidResponse
                }
                
                let result = try JSONDecoder().decode(ChatGPTRecommendation.self, from: jsonData)
                
                let ownedBooks = result.ownedBooks.map {
                    // TODO: 로컬저장소 중에 $0과 일치하는 이름의 id값 찾아서 return
                    return $0
                }
                
                let newBooks = result.newBooks.map {
                ...
                retryCount += 1
                print("Retry attempt in getBookRecommendation \(retryCount): \(error.localizedDescription)")
                continue
            }
        }
    ...
    private func fetchSearchResults(_ query: String) async throws -> [BookItem] {
        ...
        guard
            let clientId = loadEnv()?["NAVER_CLIENT_ID"],
            let clientSecret = loadEnv()?["NAVER_CLIENT_SECRET"],
        ...
        try await Task.sleep(nanoseconds: 1_000_000_000 / 8) // MARK: 재시도로 인한 속도 제한 에러 방지
        var request = URLRequest(url: url)
